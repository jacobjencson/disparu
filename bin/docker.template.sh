...
if [[ ${verbose} -eq 1 ]]; then
  write_magenta "%% bash $0 --command=${docker_cmnd} --file=${docker_file} --name=${docker_name} --port=${docker_port} --tag=${docker_dtag} --dry-run=${dry_run} --verbose=${verbose}"
...
# -
_disparu () {
  # _disparu ${dry_run} ${docker_port}
  if [[ ${verbose} -eq 1 ]]; then
    write_magenta "_disparu(${1}, ${2})"
  fi
  if [[ ${1} -eq 1 ]]; then
    write_yellow "Dry-Run>> docker run -d --name=${my_username} -p ${2}:5432 -e POSTGRES_USER=${my_username} -e POSTGRES_PASSWORD=${my_password} -e PGDATA=/pgdata -v ${my_volume}/pgdata/${my_username}:/pgdata -v ${my_volume}/pgdata/backups:/backups ${my_image}"
  else
    write_green "Executing>> docker run -d --name=${my_username} -p ${2}:5432 -e POSTGRES_USER=${my_username} -e POSTGRES_PASSWORD=${my_password} -e PGDATA=/pgdata -v ${my_volume}/pgdata/${my_username}:/pgdata -v ${my_volume}/pgdata/backups:/backups ${my_image}"
    docker run -d --name=${my_username} -p ${2}:5432 -e POSTGRES_USER=${my_username} -e POSTGRES_PASSWORD=${my_password} -e PGDATA=/pgdata -v ${my_volume}/pgdata/${my_username}:/pgdata -v ${my_volume}/pgdata/backups:/backups ${my_image}
  fi
}
